...
Tampermonkey Chrome linkini aç
"Chrome'a ekle" butonuna bas
"Uzantıyı ekle" diyerek onayla
Uzantılardan geliştirici modunu aç
Yine uzantılardan tampermonkey ayrıntılarına tıklayıp "Kullanıcı komut dosyalarına izin ver"
Tampermonkey ayarlar dan Yapılandırma modu: Gelişmiş seç
Firefox için:

Tampermonkey Firefox linkini aç
"Firefox'a ekle" butonuna bas
Safari için:

App Store'dan Tampermonkey arayıp indir
Adım 2: Script'i Kopyala
GitHub'da bir script dosyasına tıkla (örnek: EtsyFinans.user.js)
"Raw" butonuna bas (aslında Raw a basınca otomatik yükler (3.adıma gerek kalmaz.) yükle ..., eğer yüklemezse sonraki adım.)
Sayfanın tamamını seç (Ctrl+A veya Cmd+A)
Kopyala (Ctrl+C veya Cmd+C)
Adım 3: Tampermonkey'e Yapıştır
Tarayıcının sağ üstünde Tampermonkey simgesine tıkla
"Yeni script oluştur" seçeneğini tıkla (veya "Create a new script")
Varsayılan metni sil
Kopyaladığın kodu yapıştır (Ctrl+V)
...
Script'i kurunca Tampermonkey'deki listede görürsün
Etsy hesabında "Finans /Monthley Statements" sayfasına git
Sayfayı yenile
Sonuç: Seçtiğin ay kaç lira kazandığını,reklam ve fee yüzdelerini göreceksin ✨
...
EtsyReviewMessage script'i kur
Etsy'de "Orders/comlated delivered" sayfasına git. Review mesajı göndermek istiyorsan şablonu tampermonkey simgesinden ayarlarından mesaj şablonları ayarla.
Sırayla review mesajı gmndermek için ctrl+spaca (mesaj yazar) , ctrl+alt (mesaj gönderir), ctrl+ sağ tuş(diğer siparişe geçer)
Orders sayfasında müşteri mesajını seçince 10 çeşit mesaj gönderebilirsin. ctrl + sayı.
Beğendiğin cevap şablonunu seç
İhtiyacına göre düzenle ve gönder
Zaman kazan: Her yorum için 5 dakika yerine 30 saniye! ⚡
Senaryo 3: Reklam Sözcüklerini Listele
Ne yapacaksın: Ürünlerin hangi arama kelimeleriyle bulunduğunu göreceksin.

Adımlar:

AdWordlist script'i kur
Ürün reklamlarnın bulunduğu sayfaya git
Sayfasaki listingleri aç.
ctrl+alt otomatik gereksiz kelimeleri kapatır.
Fayda: gereksiz harcama yapan kelimeleri kapatmış olursun. Ayarlarından bunları düzenleyebilirsin. Biliyorsan regex de yapabilirsin. 🎯
Senaryo 4: Resimleri Hover Ettiğinde Önizle
Ne yapacaksın: Ürün resimleri hakkında daha fazla bilgi göreceksin.

Adımlar:

EtsyImageHoverPreview script'i kur
Etsy'de ürün listelemedeki küçük resimlerin üzerine mouse'u getir
Resim büyüyüp daha net görünecek ✨
Fayda: Resimleri tıklamadan preview görebilirsin (zaman tasarrufu)
🐛 Sorun Giderme
...
💡 İpuçları
Hepsini birden kurma - Bir iki tane ile başla, alışkanlık yap, sonra diğerlerini ekle
Tarayıcı konsolunu aç (F12 → Console) - Script hataları burada görürsün
Etsy'yi yenile - Script değişiklikler tarayıcı yenilendikten sonra uygulanır
Tampermonkey ayarlarını kontrol et - Sağ tık → "Tampermonkey" → "Yönet"
📞 Yardım ve Destek
